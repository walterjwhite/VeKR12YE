...
prepare_lib() {
  ...
  # If lib not found locally, try to use the already cloned repository
  if [ -z "$LIB_SOURCE" ]; then
    # Check if lib exists in the already cloned repository root (TEMP_CLONE_DIR)
    if [ -n "${TEMP_CLONE_DIR:-}" ] && [ -d "$TEMP_CLONE_DIR/lib" ] && [ -f "$TEMP_CLONE_DIR/lib/package.json" ]; then
      LIB_SOURCE="$TEMP_CLONE_DIR/lib"
      log "Using lib from already cloned repository: $LIB_SOURCE"
    # Otherwise, try to clone the repository specifically for lib
    elif [ -n "${REPO_URL:-}" ] && [ "$REPO_URL" != "null" ]; then
      LIB_TEMP_CLONE=$(mktemp -d)
      chmod 755 "$LIB_TEMP_CLONE"
      log "Cloning repository from $REPO_URL to fetch lib..."
      if git -c safe.directory='*' clone --depth 1 --single-branch "$REPO_URL" "$LIB_TEMP_CLONE"; then
        if [ -d "$LIB_TEMP_CLONE/lib" ] && [ -f "$LIB_TEMP_CLONE/lib/package.json" ]; then
          LIB_SOURCE="$LIB_TEMP_CLONE/lib"
          log "Using lib from cloned repository: $LIB_SOURCE"
        else
          rm -rf "$LIB_TEMP_CLONE"
          LIB_TEMP_CLONE=""
          log "No lib directory found in cloned repository, continuing without @walterjwhite/lib"
        fi
      else
        rm -rf "$LIB_TEMP_CLONE"
        LIB_TEMP_CLONE=""
        log "Failed to clone repository for lib, continuing without @walterjwhite/lib"
      fi
    else
      log "No repository URL available to fetch lib, continuing without @walterjwhite/lib"
    fi
  fi
...
cleanup() {
  ...
  if [ -n "${LIB_TEMP_CLONE:-}" ] && [ -d "$LIB_TEMP_CLONE" ]; then
    rm -rf "$LIB_TEMP_CLONE"
  fi
